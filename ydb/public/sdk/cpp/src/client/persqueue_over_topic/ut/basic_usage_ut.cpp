...
//                        UNIT_FAIL("Test does not support lock sessions yet");
    ...
    void SimpleWriteAndValidateData(
        ...
        {
            auto sessionAdapter = TSimpleWriteSessionTestAdapter(session.get());
    ...
    // DISABLED: This test uses CreateSyncExecutor() for compression which causes
    // the wrapper's TFederatedSimpleBlockingWriteSessionAdapter::WaitForToken to block
    // after the first message. The SyncExecutor runs compression on the calling thread
    // which conflicts with the blocking write session event loop.
#if 0
