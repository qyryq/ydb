#include <ydb/public/sdk/cpp/src/client/persqueue_over_topic/ut/ut_utils/ut_utils.h>

#define INCLUDE_YDB_INTERNAL_H
#include <ydb/public/sdk/cpp/src/client/impl/internal/logger/log.h>
#undef INCLUDE_YDB_INTERNAL_H

#include <ydb/public/sdk/cpp/src/client/persqueue_over_topic/persqueue.h>
...
#if defined(PERSQUEUE_OVER_TOPIC_INTERNAL_TESTS)
...
#define ASSERT_BAD_VALIDATION                                           \
    ...
    // NOTE: SpecifyClustersExplicitly test is disabled for persqueue_over_topic.
    // This test tests PersQueue-specific cluster discovery (via ClusterDiscoveryService)
    // which maps cluster names (like "dc1") to endpoints.
    // The persqueue_over_topic wrapper uses FederatedTopic SDK which has a different
    // federation discovery mechanism that works with database names/paths, not cluster names.
    // Use DisableClusterDiscovery(true) when using persqueue_over_topic.
#if 0
    ...
    // DISABLED: The StopReadingData/ResumeReadingData feature in the wrapper only sets a
    // DataReadingSuspended_ flag that filters events in the adapter, but doesn't actually
    // stop the underlying FederatedTopic session from requesting data. This causes test
    // timing issues where the test expects data to stop flowing but it continues.
#if 0
