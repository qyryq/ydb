...
Y_UNIT_TEST_SUITE(CompressExecutor) {
    // DISABLED: This test uses custom executor queueing (TYdbPqWriterTestHelper) 
    // which doesn't work correctly with the persqueue_over_topic wrapper's
    // FederatedSimpleBlockingWriteSessionAdapter. The wrapper adapter's event loop
    // conflicts with the test's custom compression queue.
#if 0
...
#endif
    // DISABLED: This test uses TYdbPqTestExecutor custom compression queue with retry
    // policy testing (KickTablets), which doesn't work correctly with the wrapper's
    // write session adapter due to the adapter's different event handling.
#if 0
