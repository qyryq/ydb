...
public:
    ...
    void WriteToTopic(const TVector<TString>& data, bool compress = true) {
        // Create a driver with database="/Root" configured so FederatedTopic fallback works correctly.
        // Server.AnnoyingClient->GetDriver() may not have the database set.
        ...
        auto client = NYdb::NPersQueue::TPersQueueClient(driver);
        ...
        writer->Close();
        driver.Stop(true);  // Wait for all async operations to complete
    }
